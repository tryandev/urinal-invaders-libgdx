...
public class WorldRenderer {
	...
	public void render(World world, float deltaTime, int state) {
		...
		camera.setMatrices(gl);
			...
			renderShip(gl, world.ship, state, deltaTime);
			if (state != GameScreen.GAME_OVER) {
				renderInvaders(gl, world.invaders, deltaTime);
				renderUrinal(gl, world.invaders, deltaTime);
	...
	private void renderShip(GL10 gl, Ship ship, int state, float deltaTime) {
		if (ship.state == Ship.SHIP_EXPLODING) {
			...
			if (state != GameScreen.GAME_OVER) {
				...
				gl.glRotatef((float) Math.sin(invaderAngle*2)*45 + 45, 0, 1, 0);
	...
	private void renderInvaders(GL10 gl, List<Invader> invaders, float deltaTime) {
		...
		for (int i = 0; i < len; i++) {
			...
			if (invader.state == Invader.INVADER_DEAD) {
				...
				float boundRadius = invader.bounds.radius
						/ Invader.INVADER_RADIUS;
				gl.glPushMatrix();
	...
	private void renderUrinal(GL10 gl, List<Invader> invaders, float deltaTime) {
		...
		Assets.urinalModel.draw(GL10.GL_TRIANGLES, 0,
				Assets.urinalModel.getNumVertices());
		gl.glPopMatrix();
		Assets.invaderModel.unbind();
	}
